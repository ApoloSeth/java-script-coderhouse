...
//1
/* pelicula = movies.find(movie => movie.id === 843633)
alert(`La pelicula con id ${pelicula.id} se llama ${pelicula.original_title}`)*/

//2
/* titleUsuario = prompt("Ingrese el nombre de una pelicula: ")

for (const movie of movies){
    if (movie.original_title.toLowerCase() == titleUsuario.toLowerCase()){
        alert(`Cantidad de votos ${movie.vote_count}`)
        break
    }
}*/

//3
/*const peliculasPopulares = movies.filter(movie => movie.popularity > 1000)

alert(peliculasPopulares.map(movie => movie.original_title).join("\n"))*/

//4
/*const titleConFecha = movies.map(movie => `Titulo pelicula: ${movie.original_title}, fecha: ${movie.release_date}`)
alert(titleConFecha.join("\n"))*/

//5
/*const ordenPorPolularidadAscendente = movies.sort((a,b) => a.popularity - b.popularity)
console.log(ordenPorPolularidadAscendente)*/

//6
/*const OrdenPorPopularidadDescendente = movies.sort((a,b) => b.popularity - a.popularity)
console.log(OrdenPorPopularidadDescendente)*/
...
//9
/*const arrayReducido = movies.map(function (movie) {
    return {title: movie.original_title, id: movie.id, popularit: movie.popularity}
})*/

//10
/*const arrayConView = arrayReducido.map(function (movie){
    return {title: movie.title, id: movie.id, popularit: movie.popularit, view: 0}
})
console.log(arrayConView)*/

//11
const arrayConAdult = movies.map(function (movie){
    if (movie.genre == "Horror") {
        return {title: movie.original_title, id: movie.id, popularit: movie.popularity, view: 0, adult: true}
    } else if (movie.genre == "Thriller"){
        return {title: movie.original_title, id: movie.id, popularit: movie.popularity, view: 0, adult: false}
    } else {
        return {title: movie.original_title, id: movie.id, popularit: movie.popularity, view: 0, adult: "N/A"}
    }    
})
console.log(arrayConAdult)
